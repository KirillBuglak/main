...
public class Main {
    ...
    private static RouteCalculator getRouteCalculator() {
        ...
        return new RouteCalculator(createStationIndex());
    }
    ...
    private static Station takeStation(String message) {
        for (; ; ) {
            ...
            logger.info("People are searching for - " + line);
            Station station = stationIndex.getStation(line);
            if (station != null) {
                return station;
            }
            logger.warn("No such station - " + line);
            System.out.println("Станция не найдена :(");
        }
    }

    private static StationIndex createStationIndex() {
        stationIndex = new StationIndex();
        try {
            JSONParser parser = new JSONParser();
            JSONObject jsonData = (JSONObject) parser.parse(new FileReader(DATA_FILE));
